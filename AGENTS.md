...
### P0 - Security & Data Integrity (NEVER VIOLATE)
...
|-------|-------|-------------|
| **P0** | Security & Auth | Never compromise. Stop and flag if requirement conflicts. |
| **P1** | Correctness & Testing | Test coverage mandatory. Bugs fixed before features. |
...
### Simplicity
- **Reference working code**: If DB connection fails, find `WorkingService.py` that works, copy pattern
